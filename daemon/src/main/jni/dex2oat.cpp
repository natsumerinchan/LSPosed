...
Java_org_lsposed_lspd_service_Dex2OatService_doMountNative(JNIEnv *env, jobject,
                                                           jboolean enabled, jstring magiskTmp,
    ...
    if (pid_t pid = fork(); pid > 0) { // parent
        ...
        std::string dex2oat32p, dex2oat64p;
        if (magiskTmp) {
            std::string base = env->GetStringUTFChars(magiskTmp, nullptr);
            base.append("/.magisk");
            dex2oat32p = base + (dex2oat32 + strlen("/data/adb"));
            dex2oat64p = base + (dex2oat64 + strlen("/data/adb"));
        } else {
            dex2oat32p = dex2oat32;
            dex2oat64p = dex2oat64;
        }

        if (enabled) {
            LOGI("Enable dex2oat wrapper");
            if (r32) mount(dex2oat32p.c_str(), r32p, nullptr, MS_BIND, nullptr);
            if (d32) mount(dex2oat32p.c_str(), d32p, nullptr, MS_BIND, nullptr);
            if (r64) mount(dex2oat64p.c_str(), r64p, nullptr, MS_BIND, nullptr);
            if (d64) mount(dex2oat64p.c_str(), d64p, nullptr, MS_BIND, nullptr);
